...
- 原則として「ソースコードのmain関数の中に，数値そのものは書かない」ようにしてください．
  ...
  #define AVE_A 170.8
  #define AVE_B 169.7
  #define VAR_A 5.43*5.43
  #define VAR_B 5.5*5.5
  ```
  と書き`ma`などを使う部分で代わりに`AVE_A`などわかりやすい変数名を使うといいかもです。
  あと、va=5.43*5.43,vb=5.5*5.5とありますが、sqrtで平方根にするのでしたら、
  最初からva=5.43,vb=5.5にしておくと、処理の中でsqrtを使わずにすむかもです。

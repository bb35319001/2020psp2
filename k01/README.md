...
## ソースコードの説明
l.6~7:平均を計算するave＿online関数と分散を計算するvar＿online関数をプロトタイプ宣言

l.8:データ数をカウントする変数を宣言,初期化

l.9~11:平均、分散、自乗の平均を格納する変数を宣言、初期化

l.33:データが読み込まれたら1ずつ加算し、データの数を格納する

l.34~36:それぞれの関数の戻り値をそれぞれの変数に格納する

l.40~43:平均と分散、母集団の平均と分散をprintfで表示

l.57~61:平均を求める計算を戻り値として返す

l.63~66:分散を求める計算を戻り値として返す
...
- l.40-41の出力は，標本についてのものだけです．課題では，母集団の平均と分散の推定値も出力するように求めています．

[comment #20200429]

- `l.36のvar_onlineは，更新した平均，2乗平均を使ってはまずいのではないでしょうか．`と書いた前の修正指示は取り消します．青木くんの作った関数では，
  計算し終えたaveとsquare_aveを用いて計算することになっているので．前の修正指示で想定していた関数は，1つ前の平均と二乗平均を使って計算する関数でした．
  ごめんなさい．戻しておいてください．
- レポートに入出力結果を載せる場所を追加しました．実際に動かしたときの入出力の様子をコピペして載せてください．
- おそらく不偏分散の計算方法が間違っています．不偏分散は分散より大きくなるはずです．